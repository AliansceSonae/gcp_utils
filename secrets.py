...
def get_secret(client: SecretManagerServiceClient, project_number: int, name: str, version: int = 1):
...
def load_secrets(secrets_names: list, project: int) -> dict:
    ...
    for name in secrets_names:
        response[name] = get_secret(
            s_client, project_number=project, name=name)
